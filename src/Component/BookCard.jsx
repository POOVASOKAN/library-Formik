...
const BookCard = ({ book, id, dispatch }) => {
  ...
  const cardShow = !showEdit ? ( 
     ...
     <div className="flex flex-col px-3 py-2  text-white xs:px-5 xs:py-3 shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px] justify-around gap-5 h-full" id="book-card">
      <div className="flex flex-col justify-around gap-5">
      <div className="flex items-center justify-center">
      <img className="img-cover" src= "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQkhFXzAFjU9t4q6RSFDoutZu8s0V_PMovP7t0rs5-BBokZsTdAeX9yOS5JFIEgRDTVS8U&usqp=CAU"alt="Book Cover"></img>
    </div>
        <p>
        ...
        <p>
          <span className="mr-1 font-medium">Published Date:</span>
          <span>{book.date}</span>
        </p>
      </div>
  ...
  const cardEdit = showEdit ? (
    <form
      ...
      className="flex flex-col px-3 py-2  text-white xs:px-5 xs:py-3 shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px] rounded-md justify-around gap-5 h-full"
    >
      <div className="flex flex-col justify-around gap-1 flex-warp">
        <p className="flex flex-col flex-wrap items-center gap-1">
          <span className="mr-1 font-medium whitespace-nowrap">Title:</span>
          <input
            type="text"
            name="title"
            value={values.title}
        ...
        <p className="flex flex-col flex-wrap items-center gap-1">
          ...
          {touched.author && errors.author ? (
            <span className="text-xs text-red-700">{errors.author}</span>
          ) : (
            ""
          )}
        </p>
        <p className="flex flex-col flex-wrap items-center gap-1">
          ...
          {touched.isbn && errors.isbn ? (
            <span className="text-xs text-red-700">{errors.isbn}</span>
          ) : (
            ""
          )}
        </p>
        <p className="flex flex-col flex-wrap items-center gap-1">
          ...
          {touched.date && errors.date ? (
            <span className="text-xs text-red-700">{errors.date}</span>
          ) : (
            ""
          )}
        </p>
