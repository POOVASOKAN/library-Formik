...
const AuthorCard = ({ author, id, dispatch }) => {
  ...
  const authorShow = !showEdit ? (
    <div className="flex flex-col px-3 py-2 text-white xs:px-5 xs:py-3 shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px] rounded-md justify-around gap-5 h-full overflow-hidden">
      <div className="flex flex-col justify-around gap-5">
        <div className="flex items-center justify-center">
          <img
            className="author-cover"
            src="https://static.wixstatic.com/media/4d3f81_f0464c88ab894eadbe306433a53c10c7~mv2.png/v1/crop/x_0,y_106,w_4000,h_1548/fill/w_636,h_244,al_c,q_85,usm_0.66_1.00_0.01,enc_auto/Untitled%20design%20(66).png"
            alt="Book Cover"
          ></img>
        </div>
        <p>
          <span className="mr-1 font-medium">Author Name:</span>
          <span>{values.authorName}</span>
        </p>
        <p>
          <span className="mr-1 font-medium">Birth Date:</span>
          <span>{values.authorBirth}</span>
        </p>
        <p>
          <span className="mr-1 font-medium">Biography:</span>
  ...
  const authorEdit = showEdit ? (
    <form
      ...
      className="flex flex-col px-3 py-2 text-white xs:px-5 xs:py-3 shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px] rounded-md justify-around gap-5 h-full min-h-max"
    >
      <div className="flex flex-col justify-around gap-5">
        <p className="flex flex-col flex-wrap items-center gap-1">
          ...
          {touched.authorName && errors.authorName ? (
            <span className="text-xs text-red-700">{errors.authorName}</span>
          ) : (
            ""
          )}
        </p>
        <p className="flex flex-col flex-wrap items-center gap-1">
          ...
          {touched.authorBirth && errors.authorBirth ? (
            <span className="text-xs text-red-700">{errors.authorBirth}</span>
          ) : (
            ""
          )}
        </p>
        <p className="flex flex-col flex-wrap items-center gap-1 h-26 min-h-max">
          <span className="mr-1 font-medium whitespace-nowrap">Biography:</span>
          <textarea
            type="text"
            name="authorBio"
          ...
          {touched.authorBio && errors.authorBio ? (
            <span className="text-xs text-red-700">{errors.authorBio}</span>
          ) : (
            ""
          )}
        </p>
